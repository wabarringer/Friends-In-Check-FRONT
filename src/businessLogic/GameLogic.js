...
class GameLogic {
  ...
  onSquareClicked(index) {
    ...
    if (!this.processingMessage && this.currentGameState.currentColorTurn != this.playerColor) {
      return;
    }
    ...
    if (!square.piece) {
      ...
      if (this.currentGameState.selectedPiece) {
        this.log("there is currently a selected piece ",this.currentGameState.selectedPiece)

        // and target is in a list of allowed moves
        if (this.currentGameState.allowedSquares.indexOf(index) > -1) {
          // then move piece to target
          this.log("target square is in the list of allowed squares ",this.currentGameState.allowedSquares)
          this.onPieceMoved(square);
        } else {
          // otherwise unselect piece
          this.log("target square is NOT in the list of allowed squares ",this.currentGameState.allowedSquares)
    ...
    if (!this.currentGameState.selectedPiece && 
      ...
      this.log("there is no selected piece and the current player color is not the piece's color. piece:",piece)
      return;
    }
    ...
    if (this.currentGameState.currentColorTurn == piece.color)
    {
      ...
      this.log("selecting target piece because the target piece is the current player's color.",piece)
